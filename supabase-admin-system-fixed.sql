...
-- Function to approve a community spot (fixed parameter types)
CREATE OR REPLACE FUNCTION approve_community_spot(
  spot_id TEXT,
  admin_id TEXT  -- Changed back to TEXT to match JavaScript string
)
RETURNS BOOLEAN AS $$
BEGIN
  -- Check if user is admin (cast TEXT to UUID)
  IF NOT is_admin(admin_id::UUID) THEN
  ...
  SET 
    status = 'approved',
    approved_by = admin_id::UUID,  -- Cast TEXT to UUID
    approved_at = NOW(),
    updated_at = NOW()
  WHERE id = spot_id;
  ...
  VALUES (admin_id::UUID, 'approve_spot', spot_id, 'community_spot', '{"action": "approved"}');

  RETURN true;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- Function to reject a community spot (fixed parameter types)
...
BEGIN
  ...
  -- Log the admin action
  INSERT INTO admin_actions (admin_id, action_type, target_id, target_type, details)
  VALUES (admin_id::UUID, 'reject_spot', spot_id, 'community_spot', 
...
-- Make the first user an admin (you can change this email to your admin email)
